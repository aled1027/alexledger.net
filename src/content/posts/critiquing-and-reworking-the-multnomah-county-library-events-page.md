...
https://multco-library-events.netlify.app/
...
I researched events pages at other libraries across the world, and I was surprised by the general low quality. There was lots of use of basic cards, like the kind of cards that come out of the box with a component library, to show events in a list, which makes it hard for eyes to skim find relevant events.

The public website I liked the most upon scanning was the city of Berlin's website, https://www.berlin.de/en/.
I love the search bar at the start - get me going somewhere, ala google's search design. And the animated header with Berlin-related imagery, like the standing bear.
...
I went forward with building my own version of the event pages.

A few ideas I had going in:

1. Make the filters apply instantly
2. Support a nice, full-text search, with prefix search, so "bili" returns bilingual
3. Make it easy and obvious to filter to locations

Design & Implementation Decisions:

1. I used checkboxes instead of drop-downs where possible.

I've noticed that less computer-savvy patrons tend to struggle with drop-downs because they need to click the dropdown, and then click the item. Often they butter-finger (is that term?) and select the wrong thing, which means they need to do the process again. I like checkboxes for this use-case because if they butter-finger and click the wrong thing, then (a) the remediation is to do the same action, as opposed to trying the drop-down flow again, and (b) they can click the location they want and worst case they get results for two locations instead of one.
...
I scraped the library events for August 14, 2024, through October 14, 2024, using a python script. I stored the events in a JSON file that's read client-side to render the events. Even with all that work, this implementation got near full 100s in lighthouse.

[^1]: There's probably an actual name for these, but I'll call them hot filters because they're fast, easy-to-apply filters, often based on search trends.
